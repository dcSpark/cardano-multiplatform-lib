...
#[derive(Debug, Clone)]
pub struct ByronProxySecretKey {
    pub omega: u64,
...
impl cbor_event::se::Serialize for ByronProxySecretKey {
    fn serialize<'se>(
        &self,
...
impl cbor_event::de::Deserialize for ByronProxySecretKey {
    fn deserialize(raw: &mut Deserializer) -> cbor_event::Result<Self> {
        raw.tuple(4, "ByronProxySecretKey")?;
...
impl ByronProxySecretKey {
    ...
    fn data_to_sign(
        ...
        let mut buf2 = vec![b'0', b'0'];
        buf2.extend(delegate_pk.as_ref());
        let mut ser2 = se::Serializer::new(buf2);
...
mod tests {
    ...
    #[test]
    fn test_psk_verify() {
        let mut psk = ByronProxySecretKey {
            omega: 0,
            issuer_pk: PublicKey::<Ed25519Bip32>::from_binary(&STANDARD.decode("nFhj99RbuDjG5jU3XjRXlUbP+4LStPeiMh7E7l3oWWfwRqjxXg10jUFt+4pKRlnZTrmI4weBWMGpchDJA9MKnA==").unwrap()).unwrap(),
            delegate_pk: PublicKey::<Ed25519Bip32>::from_binary(&STANDARD.decode("mLujHvc/6KIvUEt2IdnjmVRENEHx9ifl45ZmhZZ8e39+C4fe/HgnKjFtT1M5LjeeSn1Bp8tSAM4WZwL+ECWgsw==").unwrap()).unwrap(),
    ...
    fn test_psk_sign() {
        ...
        let psk = ByronProxySecretKey::sign(&issuer_prv, delegate_pk, 0, pm);
