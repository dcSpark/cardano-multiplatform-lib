    ...
    ($name:ident, $primitive:ident, $wasm_mod:ident) => {
        ...
        impl $name {
            ...
            pub fn new(primitive: &$wasm_mod::$primitive) -> Self {
                Self(core_crypto::chain::ChainCrypto::from(
                    primitive.as_ref().clone(),
                ))
            }
            ...
            pub fn from_raw_bytes(bytes: &[u8]) -> Result<$name, JsError> {
                core_crypto::chain::ChainCrypto::<core_crypto::$primitive>::from_raw_bytes(bytes)
                    .map(Self)
                    .map_err(Into::into)
            }
