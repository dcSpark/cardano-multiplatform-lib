...
use super::witness_builder::RedeemerWitnessKey;
...
impl TransactionBuilder {
    ...
    fn build_and_size(&self) -> Result<(TransactionBody, usize), JsError> {
        ...
        for redeemer in self.witness_builders.redeemer_set_builder.build().0.iter() {
        ...
        let script_data_hash = match has_dummy_exunit {
            true => Some(ScriptDataHash::from([0u8; ScriptDataHash::BYTE_COUNT])),
            false => calc_script_data_hash(
                &self.witness_builders.redeemer_set_builder.build(),
    ...
    pub fn build_body(&self) -> Result<TransactionBody, JsError> {
...
mod tests {
    ...
    fn test_contract_dummy_exunit() {
        ...
        let real_script_hash = tx_builder.build_body().unwrap().script_data_hash().unwrap();
    ...
    fn build_tx_with_ref_input() {
        ...
        assert_eq!(tx_builder.outputs.len(), 2);
        let final_tx = tx_builder.build().unwrap().build_unchecked();
